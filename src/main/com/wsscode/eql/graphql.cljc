...
(defn ast->graphql [{:keys   [type children key dispatch-key params union-key query]
  ...
  (letfn [(continue
            ([x] (continue x inc))
            ([x depth-iterate]
             (ast->graphql (assoc x ::depth (depth-iterate depth)
...
(comment
  (str/join (repeat 1 "  "))
  (println (eql->graphql '[({:all [:id :name]}
                            {:last "csaa"})] {}))

  (eql/query->ast '[(call-something {:a 1 :b {:c 3}})])
  (ident-transform [:Counter/by-id 123])
  (println (eql->graphql [{[:Counter/by-id 123] [:a :b]}])))
